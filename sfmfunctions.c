...
inline bool get_message(struct string* message, int socket_fd) //create buffer for realloc_read and read the first two bytes for length of incoming string, then run realloc_read to read that amount
{
	...
	return message->data[0] == '\0' ? false : got_message;
}
...
inline void send_string(char* message, int socket_fd) //takes message that already has length prefix
{
	if(write(socket_fd, message, strlen(message+2)+3) != (signed long)strlen(message+2)+3)  //strlen has to count from offset 2, then + 2 to account for the offset
	{
		printf("Übertragung der Anmeldedaten fehlgeschlagen : %s\n", strerror(errno));
		exit(3);
	} else {
